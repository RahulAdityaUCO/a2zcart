...
export function addEventListeners() {
  Element.menuLowPrice.addEventListener("click", async () => {
    history.pushState(null, null, Routes.routePathname.SORTBYLOWPRICE);
    const label = Util.disableButton(Element.menuLowPrice);
    await sortlowprice_page();
    Util.enableButton(Element.menuLowPrice, label);
  });
}
...
export async function sortlowprice_page() {
   ...
   try {
    productList = await FirebaseController.getProductLowPrice();
