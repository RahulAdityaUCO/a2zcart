...
export function addEventListeners() {
  Element.menuSortHigh.addEventListener("click", async () => {
    history.pushState(null, null, Routes.routePathname.SORTBYHIGH);
    const label = Util.disableButton(Element.menuSortHigh);
    await sort_page();
    Util.enableButton(Element.menuSortHigh, label);
  });
}

let products;
export let cart;
export async function sort_page() {
  ...
  `;
   ...
   try {
    products = await FirebaseController.getProductHigh();
    if (cart && cart.items) {
