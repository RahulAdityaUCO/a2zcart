...
export function addEventListeners() {
  Element.menuSortLow.addEventListener("click", async () => {
    history.pushState(null, null, Routes.routePathname.SORTBYLOW);
    const label = Util.disableButton(Element.menuSortLow);
    await sortlow_page();
    Util.enableButton(Element.menuSortLow, label);
  });
}

let products;
export let cart;
export async function sortlow_page() {
  ...
  `;
   ...
   try {
    products = await FirebaseController.getProductLow();
    if (cart && cart.items) {
